...
export class CustomerSignupService {
  ...
  signupUser(userData: Signup) {
    return this.http
      .post<Signup>(`${this.url}auth/register`, userData)
      .pipe(catchError(this.errorHandler));
  }

  loginUser(userData: Signup) {
    this.http
      .post<Signup>(`${this.url}auth/login`, userData)
      .pipe(catchError(this.errorHandler))
      .subscribe(
        (res) => {
          if (res && res.accessToken && res._id) {
            if (res.isAdmin === false) {
              ...
              localStorage.setItem(
                "customer",
                JSON.stringify({ _id: res._id, accessToken: res.accessToken })
              );
  ...
  getUser(userData: Signup) {
    ...
    return this.http.get<Signup>(`${this.url}users/${userData._id}`, {
      headers: Headers,
    });
  }
